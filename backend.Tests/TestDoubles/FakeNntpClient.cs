...
public sealed class FakeNntpClient : NntpClient
{
    ...
    private int _createdStreamCount;
    ...
    public int CreatedStreamCount => Volatile.Read(ref _createdStreamCount);
    ...
    {
        Interlocked.Increment(ref _createdStreamCount);
        var stream = new TrackingMemoryStream(data.Bytes, () => Interlocked.Increment(ref _disposedStreamCount));
    ...
    {
        var header = new UsenetArticleHeader
        {
