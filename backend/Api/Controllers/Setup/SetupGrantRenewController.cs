...
[ApiController]
[Route("api/setup/grant/renew")]
[Route("api/setup/renew-grant")]
public class SetupGrantRenewController(
    ...
    IAuthFailureTracker? failureTracker = null) : BaseApiController
{
    ...
    protected override async Task<IActionResult> HandleRequest()
    {
        ...
        var request = await SetupGrantRequest.ParseAsync(HttpContext, SetupGrantOperation.Renew, HttpContext.RequestAborted).ConfigureAwait(false);
        SetupGrantResult result;
        try
        {
            result = await _setupGrantService.RenewAsync(request.Username, request.Password, HttpContext.RequestAborted)
