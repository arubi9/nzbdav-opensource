...
) : BaseStoreReadonlyCollection
{
    ...
    {
        var (dir, ctx, db, usenet, config, warming) = (
            request.Name,
            httpContext,
            dbClient,
            usenetClient,
        ...
        if (_currentPathParts.Length < DavItem.IdPrefixLength)
        {
            ...
            return new DatabaseStoreIdsCollection(dir, Path.Join(currentPath, dir), ctx, db, usenet, config, warming);
        }

        var item = await dbClient.GetFileById(request.Name).ConfigureAwait(false);
        return item == null ? null : new DatabaseStoreIdFile(item, ctx, dbClient, usenet, config, warming);
    }

    protected override async Task<IStoreItem[]> GetAllItemsAsync(CancellationToken cancellationToken)
    {
        var (ctx, db, usenet, config, warming) = (httpContext, dbClient, usenetClient, configManager, warmingService);
        if (_currentPathParts.Length < DavItem.IdPrefixLength)
            return Alphabet
                .Select(x => x.ToString())
                .Select(x => new DatabaseStoreIdsCollection(x, Path.Join(currentPath, x), ctx, db, usenet, config, warming))
        ...
        return (await dbClient.GetFilesByIdPrefix(idPrefix).ConfigureAwait(false))
            .Select(x => new DatabaseStoreIdFile(x, ctx, db, usenet, config, warming))
            .Select(x => x as IStoreItem)
            .ToArray();
    }
