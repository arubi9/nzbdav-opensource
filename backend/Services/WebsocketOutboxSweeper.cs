...
public sealed class WebsocketOutboxSweeper : BackgroundService
{
    ...
    public async Task SweepOnce(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var deleted = await dbContext.WebsocketOutbox
                .Where(x => x.CreatedAt < cutoff)
                .ExecuteDeleteAsync(cancellationToken)
                .ConfigureAwait(false);

            if (deleted > 0)
                Log.Debug("WebsocketOutboxSweeper removed {Count} expired rows.", deleted);
        }
        catch (Exception ex) when (!cancellationToken.IsCancellationRequested)
        {
            Log.Warning(ex, "WebsocketOutboxSweeper failed");
        }
    }
}
