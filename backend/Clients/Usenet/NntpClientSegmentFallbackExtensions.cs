...
public static class NntpClientSegmentFallbackExtensions
{
    ...
    )
    {
        return WithFallbackAsync(
            encodedSegmentId,
            (segmentId, ct) => usenetClient.DecodedArticleAsync(segmentId, onConnectionReadyAgain, ct),
            cancellationToken
        );
    }
    ...
    public static IEnumerable<string> FlattenSegmentIds(IEnumerable<string> encodedSegmentIds)
    {
        foreach (var encodedSegmentId in encodedSegmentIds)
        foreach (var segmentId in NzbSegmentIdSet.Decode(encodedSegmentId))
